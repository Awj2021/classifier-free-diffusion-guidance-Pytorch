...
class CIFAR10(data.Dataset):
    ...
    def load_label(self):
        ...
        if isinstance(noise_label, dict):
            ...
            if self.noise_type == 'multi_rater':
                noise_multi_rater = np.stack((noise_label['random_label1'], noise_label['random_label2'], noise_label['random_label3']),axis=-1)
                ...
                return noise_multi_rater.reshape(-1, 3), noise_multi_rater_diff.reshape(-1, 3), indices
            else:
    ...
    def __getitem__(self, index):
        ...
        if self.target_transform is not None:
            if self.train:
                if self.noise_type == 'multi_rater':
                    target = [self.target_transform(target[i]) for i in range(len(target))]
                else:
